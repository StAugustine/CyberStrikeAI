...
桌面 profile 始终托管本机 Go sidecar，只绑定随机 IPv4 回环端口，并保留本地单管理员登录、权限中间件、HITL 和审计。它不注册或打包 Terminal、WebShell、C2、机器人、平台多用户 RBAC 和远程服务模式；这些能力仍可能存在于共享 server 源码中，但不属于桌面发布物。

浏览器/Burp 本地发现默认关闭，只返回短时 `127.0.0.1` 地址和实例元数据，不返回密码、Token 或凭据。发现不是认证边界，插件仍需正常登录。AI secret 由操作系统凭据库存储，配置文件只保留引用；替换程序目录不会删除数据或凭据。当前 ZIP 未签名，不应通过关闭 Windows/macOS 安全保护来运行公开分发包。
