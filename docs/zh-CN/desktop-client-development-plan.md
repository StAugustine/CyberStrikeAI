...
执行状态：进行中。已完成本地管理员登录/退出/刷新校验、AI 通道安全配置及 401/429/500/流中断脱敏失败路径、对话分组 CRUD、受管目录附件上传/下载/Agent 引用、单 Agent 与 deep/plan_execute/supervisor 三种多 Agent、取消、HITL 拒绝恢复、真实工具执行、监控详情、通知已读，以及 core 重启后旧会话失效、分组/附件/对话数据保留的桌面黄金路径；主题、国际化、安全 Markdown 和长流首事件实时送达已纳入门禁。仍需完成其余页面交互、睡眠唤醒和 Windows/macOS WebView 人工一致性验收。
...
执行状态：主体功能验收尚未开始；能力边界前置门禁已完成。desktop 模式已从活动服务、HTTP 路由、Agent 工具、导航、设置、页面和脚本加载中排除 Terminal、WebShell、C2、机器人和平台多用户 RBAC，并在 R1 隐藏 R2 API 文档入口；共享 server 模式保持原行为。
