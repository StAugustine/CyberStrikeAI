...
执行状态：进行中。已建立桌面专用一致性升级备份底座和固定备份目录：配置及受管数据按白名单复制，SQLite 主库与未合并 WAL 先在暂存区形成稳定副本，再通过 SQLite 在线备份 API 生成完整性检查通过的单文件快照；备份清单记录来源/目标版本、时间、文件类型、权限、大小与 SHA-256。备份在清单和全部载荷复核后才原子发布，符号链接、损坏数据库、上下文中断或载荷篡改均有自动化失败验收且不遗留半成品，异常退出遗留 WAL 的源文件在备份前后哈希不变。尚需接入启动升级事务、恢复与保留策略，以及旧实例导入流程和界面。
