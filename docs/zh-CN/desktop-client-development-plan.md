...
执行状态：进行中。已完成本地管理员登录/退出/刷新校验、AI 通道安全配置及 401/429/500/流中断脱敏失败路径、单 Agent 与 deep/plan_execute/supervisor 三种多 Agent、取消、HITL 拒绝恢复、真实工具执行、监控详情、通知已读、core 重启后旧会话失效与数据保留的桌面黄金路径；主题、国际化、安全 Markdown 和长流首事件实时送达已纳入门禁。仍需完成其余页面交互、睡眠唤醒和 Windows/macOS WebView 人工一致性验收。
