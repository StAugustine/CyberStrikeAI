...
当前交付一个可解压运行、可替换升级且删除程序目录不删除用户数据的 CyberStrikeAI 免安装桌面应用。用户不需要手动启动 Web 服务或寻找浏览器地址，即可在桌面窗口中使用当前项目的核心能力；业务数据、用户配置和自定义扩展在替换升级后仍然保留。安装器按项目所有者决定暂缓。

### 2.2 目标平台支持矩阵

| 平台 | 架构 | 最低目标 | 发布物 | 支持级别 |
| --- | --- | --- | --- | --- |
| Windows | x86_64 | Windows 10/11，WebView2 Evergreen | 未签名免安装 ZIP 开发候选 | 一级支持 |
| macOS | arm64 | macOS 12+ | 未签名、未公证 `.app` 免安装 ZIP 开发候选 | 一级支持 |
| macOS | x86_64 | macOS 12+ | 未签名、未公证 `.app` 免安装 ZIP 开发候选 | 一级支持 |
| Windows | arm64 | 不纳入首个正式版本 | 无 | 后续评估 |
| Linux | 任意 | 不纳入首批范围 | 无 | 后续评估 |

### 2.3 可验证成功标准

每一个当前发布阶段都必须满足：ZIP 可从任意用户目录解压运行、替换升级且删除程序目录后用户数据仍保留，不依赖仓库目录；管理服务仅绑定 `127.0.0.1` 随机端口；不绕过认证、授权、HITL 和审计；数据升级失败可恢复；关闭后无 sidecar 或该阶段子进程遗留；Windows 与 macOS 目标均有原生 runner 免安装验收证据；现有 `cmd/server` 和 API 保持兼容。
...
执行状态：R1 当前免安装开发交付范围已完成。已完成本地管理员登录/退出/刷新校验、AI 通道安全配置及 401/429/500/流中断脱敏失败路径、对话分组 CRUD、受管目录附件上传/下载/Agent 引用、单 Agent 与 deep/plan_execute/supervisor 三种多 Agent、取消、HITL 拒绝恢复、真实工具执行、监控详情、通知已读，以及 core 重启后旧会话失效、分组/附件/对话数据保留的桌面黄金路径；仪表盘已移除并停止轮询排除模块，主题、国际化、安全 Markdown 和长流首事件实时送达已纳入门禁。Windows x64、macOS arm64 和 macOS x64 均通过实际 Tauri/WebView 启动、首次初始化、READY、退出、单实例与失败恢复冒烟；真实设备长时间睡眠/唤醒保留在最终公开稳定分发门禁。
...
执行状态：已完成。已建立桌面专用一致性升级备份底座和固定备份目录：配置及受管数据按白名单复制，SQLite 主库与未合并 WAL 先在暂存区形成稳定副本，再通过 SQLite 在线备份 API 生成完整性检查通过的单文件快照；备份清单记录来源/目标版本、时间、文件类型、权限、大小与 SHA-256。备份在清单和全部载荷复核后才原子发布，符号链接、损坏数据库、上下文中断或载荷篡改均有自动化失败验收且不遗留半成品，异常退出遗留 WAL 的源文件在备份前后哈希不变。core 启动现已在资源、配置、凭据与数据库迁移之前建立唯一恢复点和原子 pending 状态，应用就绪后、发送 `READY` 前才完成事务；中断后会校验并复用原恢复点继续幂等迁移，不会重复备份，语义版本降级则拒绝自动迁移并要求恢复匹配备份。真实 v1→v2 集成验收已覆盖资源校验失败、pending 保留、修复后续跑、旧项目数据库和用户角色备份及最终版本发布。备份目录现可逐项校验并报告有效性、版本、文件数和空间占用；策略标记至少保留最近两个有效恢复点，且未完成升级绑定的恢复点始终受保护。显式恢复会先把已校验载荷复制到同文件系统私有暂存区，再以持久化恢复状态切换配置和数据顶层目录；提交前再次按清单校验在线文件，任一步失败均回滚到恢复前布局，core 也会在创建目录或启动迁移前自动回滚中断事务。成功恢复、部分切换中断、暂存载荷篡改、损坏备份与保留策略均已有自动化验收。旧实例导入底座现支持 `v1.7.8` 至 `v1.7.x`：绝对路径、路径逃逸、符号链接、特殊文件和不支持版本会在预检阶段拒绝；配置、WAL 数据库、附件、检查点和五类资源按白名单映射到私有导入恢复点，远程服务凭据及排除能力不进入新配置，敏感字段仅以字段名出现在报告中，数据库副本会收敛为单本地管理员。用户确认前当前桌面数据不变；离线确认先建立当前桌面恢复点再复用可回滚恢复事务切换，取消只删除本次导入快照，篡改快照会在切换前拒绝。真实导入、失败不变、重启续接、取消和恢复原桌面均已有自动化验收。桌面界面现已接入原生目录选择、导入预检与确认、恢复点目录、显式恢复和保留策略约束下的删除；源路径不进入 WebView，维护窗口没有通用 shell 权限，替换数据前由 Tauri 状态机等待 core 正常退出，操作完成或失败后均重启 core，维护期间拒绝退出以避免留下无人管理的写事务。备份、导入与恢复现在会在暂存或切换前按目标卷检查空间，含 SQLite 双快照峰值和安全余量；系统凭据库首次及部分写入失败均回滚且保留可重试配置。Windows x64、macOS arm64 和 macOS x64 免安装候选又均使用压缩包内 sidecar 完成真实恢复、恢复前恢复点复核、程序目录替换和数据保留验收，证据见 [D6 数据恢复报告](desktop-client-d6-data-recovery.md)。
...
执行状态：当前免安装开发交付范围已完成。按项目所有者决定，R1 交付 Windows x64、macOS arm64 和 macOS x64 三个免安装 ZIP，安装器暂缓。Windows ZIP 由原生 release 可执行文件、同架构 Go sidecar、版本化默认资源、许可证和使用说明组成；macOS ZIP 包含同样完整的 `.app`、许可证和说明。候选构建显式禁用签名和更新安装产物，已实现产品版本/许可证/Bundle 配置一致性校验、桌面图标集、CycloneDX 1.6 SBOM、SHA-256 清单、资源清单哈希复核、敏感文件/私钥扫描和排除前端资源审计。macOS arm64 已在本机实际生成免安装 ZIP；Windows x64、macOS arm64 和 macOS x64 又在三个干净原生 runner 上全部通过安全解压路径、主程序与 sidecar 架构、打包后真实恢复及恢复前恢复点校验、删除程序目录不删除外置用户数据、重新解压后复用同一数据目录、内容审计、SBOM 和校验和验收。R1 功能矩阵同时在三个原生 runner 上通过 Go 黄金路径、Web/Rust 测试和 Tauri 生命周期/失败冒烟。证据见 [R1 免安装候选验收报告](desktop-client-r1-acceptance.md)、[免安装发布流水线](https://github.com/StAugustine/CyberStrikeAI/actions/runs/30684180216)和[基础桌面三平台流水线](https://github.com/StAugustine/CyberStrikeAI/actions/runs/30684180212)。项目所有者已批准继续 R2；代码签名、公证和真实设备长时间睡眠/唤醒仍是公开稳定分发门禁，未签名 ZIP 只作为开发候选。
...
执行状态：已完成。桌面端已提供默认关闭的显式联动开关、90 秒私有发现文件、Chrome/Edge 固定 ID 原生消息宿主和进程重启/失联清理；浏览器扩展与 Burp 均增加 **Use Desktop**，只接收短时回环路由元数据并继续使用原登录/授权/审计链；桌面 CORS 精确固定官方扩展 Origin，API 文档恢复为本地同窗口入口。发布流水线同时构建 native host、浏览器 ZIP、Burp JAR 和校验和。本机门禁及 Windows x64、macOS arm64、macOS x64 三个干净 runner 均通过，详见 [D8 本地插件联动报告](desktop-client-d8-plugin-integration.md)、[基础桌面流水线 30685977378](https://github.com/StAugustine/CyberStrikeAI/actions/runs/30685977378)和[插件/免安装流水线 30685977367](https://github.com/StAugustine/CyberStrikeAI/actions/runs/30685977367)。
...
- 用 R2 解压目录替换 R1 程序目录，验证原数据、配置和凭据保留。
