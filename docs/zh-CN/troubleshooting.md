...
- Windows：确认系统已有 WebView2 Evergreen Runtime。首次运行时杀毒软件可能扫描未签名的 Go core/native-host sidecar，表现为启动延迟；等待扫描结束并查看桌面日志，不要把程序加入全局排除项。
- macOS：当前开发候选未签名和公证，Gatekeeper 可能阻止打开。不要关闭系统安全保护；公开分发应使用 Developer ID 签名和 notarization。
- 替换升级：先完全退出，只替换 ZIP 解压出的程序目录，不删除操作系统应用数据/配置目录。若新版本失败，保留数据和自动生成的升级恢复点，从启动错误页进入“数据导入与恢复”。
- 插件：桌面端先显式启用“本地插件联动”，再在浏览器/Burp 点击 **Use Desktop**；发现成功后仍需输入本地管理员密码并 **Validate**。实例重启或发现信息过期后需要重新发现。

桌面版不提供 Terminal、WebShell、C2、机器人、平台多用户 RBAC 或远程服务模式。对应页面或 API 返回 404 是范围边界，不是启动故障。
